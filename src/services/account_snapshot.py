...
def _build_missing_price_review_reason(missing_symbols: List[str]) -> Optional[str]:
    ...
    Each list is summarized past ``_MAX_REASON_SYMBOLS`` ("... (+N more)") for
    readability; the column is TEXT so length isn't a constraint.
    """
    ...
    parts = []
    if options:
        pretty = _summarize_symbols([_format_symbol_for_review(s) for s in options])
        parts.append(f"[stale-options] {pretty}")
    if stocks:
        parts.append(f"Missing price data for: {_summarize_symbols(stocks)}")
    return " | ".join(parts)


# Cap on how many symbols a review_reason enumerates before summarizing the rest.
_MAX_REASON_SYMBOLS = 12


def _summarize_symbols(items: List[str]) -> str:
    """Join ``items`` with commas, collapsing the tail past the cap to
    "(+N more)" so a large stale book yields a readable reason."""
    if len(items) <= _MAX_REASON_SYMBOLS:
        return ", ".join(items)
    shown = ", ".join(items[:_MAX_REASON_SYMBOLS])
    return f"{shown} (+{len(items) - _MAX_REASON_SYMBOLS} more)"


# Markers identifying a review_reason auto-generated by the missing-price fallback, so a later fully-priced recalc clears only its own flags.
_MISSING_PRICE_MARKERS = ("Missing price data for:", "[stale-options]")


def _is_missing_price_review_reason(reason: Optional[str]) -> bool:
    """True if `reason` was auto-generated by the missing-price fallback."""
    if not reason:
        return False
    return any(marker in reason for marker in _MISSING_PRICE_MARKERS)
    ...
    while current_date <= end_date:
        try:
            ...
            if existing_snapshot:
                ...
                elif _is_missing_price_review_reason(existing_snapshot.review_reason):
                    # Prices all resolved on this pass — clear the stale missing-price flag a prior failed fetch left behind.
                    existing_snapshot.needs_review = False
                    existing_snapshot.review_reason = None
