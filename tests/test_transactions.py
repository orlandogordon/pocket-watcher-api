...
def test_update_transaction_200(client, db, test_user):
    ...
    resp = client.put(f"/transactions/{txn.id}", json={"amount": "99.99", "description": "new"})
    assert resp.status_code == 200
    body = resp.json()
...
def test_update_clear_category(client, db, test_user):
    ...
    txn = make_transaction(db, test_user, acct, category_id=cat.id)
    # Sending null explicitly clears the category (model_fields_set distinguishes it).
    resp = client.put(f"/transactions/{txn.id}", json={"category_uuid": None})
    assert resp.status_code == 200
...
def test_set_and_get_splits(client, db, test_user):
    ...
    resp = client.put(f"/transactions/{txn.id}/splits", json=_split_body(cat1, cat2))
    assert resp.status_code == 200
    body = resp.json()
    assert len(body["split_allocations"]) == 2
    assert body["category"] is None  # single-category assignment cleared

    got = client.get(f"/transactions/{txn.id}/splits").json()
...
def test_set_splits_sum_mismatch_400(client, db, test_user):
    ...
    txn = make_transaction(db, test_user, acct, amount=Decimal("100.00"))
    resp = client.put(f"/transactions/{txn.id}/splits", json=_split_body(cat1, cat2, "60.00", "30.00"))
...
def test_set_splits_single_allocation_422(client, db, test_user):
    ...
    assert client.put(f"/transactions/{txn.id}/splits", json=body).status_code == 422
...
def test_delete_splits_204(client, db, test_user):
    ...
    txn = make_transaction(db, test_user, acct, amount=Decimal("100.00"))
    client.put(f"/transactions/{txn.id}/splits", json=_split_body(cat1, cat2))
    assert client.delete(f"/transactions/{txn.id}/splits").status_code == 204
    assert client.get(f"/transactions/{txn.id}/splits").json() == []
...
def test_create_and_list_refund_relationship(client, db, test_user):
    ...
    resp = client.post(
        f"/transactions/{refund.id}/relationships",
        json={"to_transaction_uuid": str(purchase.id), "relationship_type": "REFUNDS"},
    )
    assert resp.status_code == 201
    rel = resp.json()
    assert rel["relationship_type"] == "REFUNDS"

    listed = client.get(f"/transactions/{refund.id}/relationships").json()
    assert len(listed) == 1
...
def test_create_relationship_unknown_target_404(client, db, test_user):
    ...
    txn = make_transaction(db, test_user, acct)
    resp = client.post(
        f"/transactions/{txn.id}/relationships",
...
def test_delete_relationship_204(client, db, test_user):
    ...
    rel = client.post(
        f"/transactions/{b.id}/relationships",
        json={"to_transaction_uuid": str(a.id), "relationship_type": "REFUNDS"},
    ).json()
...
def test_bulk_update_category(client, db, test_user):
    ...
    resp = client.patch(
        ...
        json={"transaction_uuids": [str(t1.id), str(t2.id)], "category_uuid": str(cat.uuid)},
    )
    assert resp.status_code == 200
    assert client.get(f"/transactions/{t1.id}").json()["category"]["id"] == str(cat.uuid)


def test_bulk_update_no_fields_400(client, db, test_user):
    acct = make_account(db, test_user)
    t1 = make_transaction(db, test_user, acct)
    resp = client.patch("/transactions/bulk-update", json={"transaction_uuids": [str(t1.id)]})
    assert resp.status_code == 400
...
def test_update_reassigns_account(client, db, test_user):
    ...
    resp = client.put(f"/transactions/{txn.id}", json={"account_uuid": str(a2.uuid)})
    assert resp.status_code == 200
...
def test_set_and_get_amortization_equal_split(client, db, test_user):
    ...
    resp = client.put(
        f"/transactions/{txn.id}/amortization",
    ...
    got = client.get(f"/transactions/{txn.id}/amortization").json()
...
def test_get_amortization_none_404(client, db, test_user):
    acct = make_account(db, test_user)
    txn = make_transaction(db, test_user, acct)
    assert client.get(f"/transactions/{txn.id}/amortization").status_code == 404
...
def test_amortization_conflicts_with_splits_400(client, db, test_user):
    ...
    resp = client.put(f"/transactions/{txn.id}/amortization", json={"start_month": "2026-01", "months": 2})
    assert resp.status_code == 400
...
def test_delete_amortization_204(client, db, test_user):
    acct = make_account(db, test_user)
    txn = make_transaction(db, test_user, acct, amount=Decimal("100.00"))
    client.put(f"/transactions/{txn.id}/amortization", json={"start_month": "2026-01", "months": 2})
    assert client.delete(f"/transactions/{txn.id}/amortization").status_code == 204
    assert client.get(f"/transactions/{txn.id}/amortization").status_code == 404
...
def test_guard_update_rejects_reassignment_to_investment(client, db, test_user):
    ...
    assert client.put(f"/transactions/{txn.id}", json={"account_uuid": str(invest.uuid)}).status_code == 400
...
def test_guard_update_allows_reassignment_to_non_investment(client, db, test_user):
    ...
    txn = make_transaction(db, test_user, checking)
    resp = client.put(f"/transactions/{txn.id}", json={"account_uuid": str(savings.uuid)})
    assert resp.status_code == 200
...
def test_guard_update_allows_field_edit_without_account_change(client, db, test_user):
    checking, _ = _guard_accounts(db, test_user)
    txn = make_transaction(db, test_user, checking)
    resp = client.put(f"/transactions/{txn.id}", json={"description": "edited"})
    assert resp.status_code == 200
...
def test_guard_bulk_update_rejects_investment_target_no_partial_write(client, db, test_user):
    ...
    resp = client.patch("/transactions/bulk-update", json={
        "transaction_uuids": [str(t1.id), str(t2.id)], "account_uuid": str(invest.uuid),
    })
    assert resp.status_code == 400
    db.refresh(t1); db.refresh(t2)
    assert t1.account_id == checking.id
    assert t2.account_id == checking.id
